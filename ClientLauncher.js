...
class MeetingLauncher {
  ...
  getRndDuration() {
    return (
      Math.floor(Math.random() * (this.MAX_ACTIVE_TIME_MS - this.MIN_ACTIVE_TIME_MS + 1)) +
      this.MIN_ACTIVE_TIME_MS
    );
  }
...
TOKEN=\`curl -X PUT "http://169.254.169.254/latest/api/token" -H "X-aws-ec2-metadata-token-ttl-seconds: 21600"\` && curl -H "X-aws-ec2-metadata-token: $TOKEN" -v http://169.254.169.254/latest/meta-data/iam/security-credentials/${role}; echo $TOKEN;`);
  ...
  getNoOfMeetingsBasedOnCoreSize() {
    ...
    if (cpuCount >= 36) {
      return Math.floor(cpuCount * 0.60);
    }
  ...
  async run() {
    if (isMainThread) {
      ...
      this.log('No Of Meetings: ' + this.NO_OF_MEETINGS);
      const threads = new Set();
      let meetingAttendeeArray = await this.createMeetingAttendeeList();
      if (meetingAttendeeArray.length > 0) {
        ...
        await this.spawnThreads(
          meetingAttendeeArray,
          threadCount,
          threads,
          loadTestStartTimeStampEpoch
        );
        this.setWorkerThreadEvents(threads);
      }
    } else {
  ...
  setWorkerThreadEvents(threads) {
    for (let worker of threads) {
      ...
      worker.on('message', async (message) => {
        ...
        this.putMetricData('ChildThreadActivityComplete', 1);
        //threads.delete(worker);
        if (threads.size === 0) {
    ...
    while (meetingAttendeeArray.length < this.NO_OF_MEETINGS * this.NO_ATTENDEES_PER_MEETING) {
      try {
        ...
        if (createMeetingWithAttendeesResponses && createMeetingWithAttendeesResponses.Messages) {
          ...
          if (Date.now() - lastMsgReceivedFromSQS > 20000) {
            meetingAttendeeArray = [];
  ...
  async childThreadActivity() {
    ...
    if (workerData.meetingAttendeeList) {
      const pages = await this.openClient();
      const now = Date.now();
      await this.resumeAudioContextForAllPages(pages);
  ...
  async openClient() {
    ...
    for (let browserTab = workerData.start; browserTab < workerData.start + workerData.range; browserTab++) {
      ...
      if (browser[browserTab] !== null && meetingInfo && attendeeInfo) {
        pages[browserTab] = await this.createNewPage(
          browser[browserTab],
          browserTab,
          mapPageMeetingAttendee
        );
        if (pages[browserTab] !== null && browser[browserTab].isConnected()) {
          this.openLinkInPage(pages[browserTab], meetingInfo, attendeeInfo, browserTab);
        }
      }
    }
    return pages;
  ...
  async createNewPage(browser, browserTab, mapPageMeetingAttendee, noOfRetries = 0) {
    ...
    if (typeof page !== 'undefined' && page !== null) {
      ...
      page.on('close', async (message) => {
        ...
        this.log(page !== null);
        if (this.loadTestEndSignal === false &&
          page !== null &&
      ...
      for (const [key, value] of Object.entries(page)) {
        this.reportFetch[key] = setInterval(async () => {
          ...
          try {
            ...
            this.error(
              'this.meetingSessionActive[browserTab] ' + this.meetingSessionActive.get(key),
              key
            );
          }
  ...
  writeMetricsToFile(webRTCStatReport, attendeeId, fileLocation) {
    ...
    if (typeof webRTCStatReport.audioPacketsReceivedFractionLoss !== 'undefined') {
      ...
      dataToWrite +=
        Math.min(Math.max(webRTCStatReport.audioPacketsReceivedFractionLoss, 0), 1) + ',';
    } else {
      dataToWrite += ',';
    }
  ...
  validateLauncherParameters() {
    ...
    if (Object.keys(launcherArgs).length > 1) {
      ...
      if (
        launcherArgs.attendeesPerMeeting &&
        typeof launcherArgs.attendeesPerMeeting !== 'number'
      ) {
      ...
      if (
        launcherArgs.metricGrabFrequencyMin &&
        typeof launcherArgs.metricGrabFrequencyMin !== 'number'
      ) {
      ...
      if (
        launcherArgs.putMetricDataNamespace &&
        typeof launcherArgs.putMetricDataNamespace !== 'string'
      ) {
